...
void localExpPipeline(const std::string inputDir, const std::string outputDir, std::string volDir, cv::Mat imL, cv::Mat imR, cv::Mat& dispL, cv::Mat& dispR, std::string KITTI_ground_truth) {
    ...
    cv::Mat I[2];
	static unary_cost_t ddr_unary_cost[WIDTH * HEIGHT * DISP_DDR_MAX];
	...
	for (int y = 0; y < HEIGHT; y++) {
		for (int x = 0; x < WIDTH; x++) {
            for (int d_ddr = 0; d_ddr < 12; d_ddr++) { // DISP_DDR_MAX
				for (int d = 0; d < 16; d++) { // DISP_DDR_NUM
					ddr_unary_cost[(y * WIDTH + x) * DISP_DDR_MAX + d_ddr](d * DISP_DDR_BIT + DISP_DDR_BIT -1, d * DISP_DDR_BIT) = ((d_ddr * DISP_DDR_NUM + d) < DISP_MAX) ? round(vol.at<float>(d_ddr * DISP_DDR_NUM + d, y, x) * DISP_DDR_VAL_MAX) : DISP_DDR_VAL_MAX;
					// assert(vol.at<float>(d_ddr * DISP_DDR_NUM + d, y, x) != std::NaN);
					// ddr_unary_cost[(y * WIDTH + x) * DISP_DDR_MAX + d_ddr](d * DISP_DDR_BIT + DISP_DDR_BIT -1, d * DISP_DDR_BIT) = vol.at<float>(d_ddr * DISP_DDR_NUM + d, y, x);
					// int d_int = d / 4;
					// int d_dec = d % 4;
					// if (d_dec == 0) { // 6bit cost
					// 	ddr_unary_cost[(y * WIDTH + x) * 12 + d_ddr](d_int * 27 + 5, d_int * 27) = 
					// 	    round(vol.at<float>(d_ddr * 16 + d, y, x) * ((1 << 6) - 1));
					// }
					// else { // 7bit cost
					// 	ddr_unary_cost[(y * WIDTH + x) * 12 + d_ddr](d_int * 27 + d_dec * 7 + 6 - 1, d_int * 27 + (d_dec - 1) * 7 + 6) = 
					// 	    round(vol.at<float>(d_ddr * 16 + d, y, x) * ((1 << 7) - 1));
					// }
				}
			}
		}
